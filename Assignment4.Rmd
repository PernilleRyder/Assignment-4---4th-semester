...
mean = aggregate(pd, by = list(pd$ID), FUN = mean)

# subsetting pitchmean, pitchsd, and ID, so we can put it into the pitch data frame
subset = subset(mean, select = c(ID,PitchMean, PitchSD))

# renaming the variables
subset = plyr::rename(subset, c("PitchMean"="PitchMean.m", "PitchSD"="PitchSD.m"))

#merging the two data sets
merged = merge(pd, subset, by= "ID")

#standardizing the standard deviation
merged$PitchSD.m.c = (merged$PitchSD.m-mean(merged$PitchSD.m))/sd(merged$PitchSD.m)
...
m2 <- map(
  alist(
    ...
    alpha ~ dnorm (0,1), #there is no difference between the diagnosis the intercept will be one
    beta ~ dnorm (0,1),
    sigma ~ dcauchy(0,2) 
  ), 
  data=merged)
...
#model with a skeptical/conservative prior 
m3 <- map(
  alist(
    PitchSD.m.c ~ dnorm(mu, sigma),
    mu <- alpha + beta * diagnosis,
